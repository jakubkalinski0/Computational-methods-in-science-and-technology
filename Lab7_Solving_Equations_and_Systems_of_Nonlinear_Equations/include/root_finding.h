...
typedef struct {
    ...
    double final_error;  /**< The value of the stopping criterion met (|x_i+1-x_i| or |f(xi)|). NAN if max iterations reached without convergence. */
 ...
 * @param precision The desired precision (rho). The method stops when BOTH
 *                  |x_{i+1} - x_i| < precision AND |f(x_{i+1})| < precision are met.
 ...
 * @param precision The desired precision (rho). The method stops when BOTH
 *                  |x_{i+1} - x_i| < precision AND |f(x_{i+1})| < precision are met.
 *                  (Note: for Secant, the first check uses the most recent two points).
 * @param max_iterations Maximum number of iterations allowed.
 * @return A RootResult struct containing the outcome (root, iterations, error, status).
 *         Status codes:
 *         0: Success, convergence criteria met.
